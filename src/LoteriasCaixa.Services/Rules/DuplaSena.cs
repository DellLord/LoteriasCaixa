...
{
    public class DuplaSena : Core.LoteriasCaixa
    {
        public DuplaSena(int numeroConcurso)
        {
        ...
        protected sealed override async Task<string> GetJogo(int numeroConcurso)
        {
            try
            {
                var url = $"{Url.DuplaSena}{numeroConcurso}";
                ...
                var collection = regex.Matches(documentText.ToSplit(3));
                ...
                sb.Append(ConcatenaNumeros);

                collection = regex.Matches(documentText.ToSplit(4));
